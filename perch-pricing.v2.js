...
// LAST UDPATE 18th of April 2025 //
...
(() => {
  ...
  function showError(errorType) {
    ...
    customPricingTags.forEach((tag) => {
      if (tag) tag.style.display = "flex"; // Changed from "block" to "flex"
    });
    hidePricing();
  }
  ...
  debugLog("Initializing Perch Pricing Calculator v2.3.3");
  ...
  // Reset all plan card selections first
  resetAllPlanCardSelections();

  if (planRadioButtons.length > 0) {
    // Find the Professional plan radio button and set it as checked
    const professionalRadioButton = Array.from(planRadioButtons).find(
      (radio) => radio.value === PROFESSIONAL_PLAN
    );
    
    if (professionalRadioButton) {
      professionalRadioButton.checked = true;
      const planCardWrapper = professionalRadioButton.closest(select("plan-card-wrapper"));
      if (planCardWrapper) {
        highlightPlanCard(planCardWrapper);
        selectedPlanDisplays.forEach((display) => {
          if (display) display.innerText = professionalRadioButton.value;
        });
      }
    }
  ...
  function resetAllPlanCardSelections() {
    debugLog("Resetting all plan card selections");
    
    // Remove highlight classes from all plan cards
    planCardWrappers.forEach((wrapper) => {
      for (let child of wrapper.children) {
        child.classList.remove(HIGHLIGHT_CLASS);
        
        // Also remove cc-highlighted class from pricing_tier_select_tag elements
        const selectTags = child.querySelectorAll('.pricing_tier_select_tag');
        selectTags.forEach(tag => {
          tag.classList.remove('cc-highlighted');
        });
      }
    });
  ...
  function highlightPlanCard(planCardWrapper) {
    debugLog("Highlighting plan card:", planCardWrapper);
    
    // First, remove highlight from all plan cards
    planCardWrappers.forEach((wrapper) => {
      for (let child of wrapper.children) {
        child.classList.remove(HIGHLIGHT_CLASS);
    ...
    // Then, add highlight to the selected plan card
    for (let child of planCardWrapper.children) {
      child.classList.add(HIGHLIGHT_CLASS);
      
      // Also add cc-highlighted class to pricing_tier_select_tag elements
      const selectTags = child.querySelectorAll('.pricing_tier_select_tag');
      selectTags.forEach(tag => {
        tag.classList.add('cc-highlighted');
      });
    }
  }
